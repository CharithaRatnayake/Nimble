...
interface NimbleAppApi {
    ...
    suspend fun getUserProfile(): UserResponseDataModel
    ...
    ): SurveyListResponseDataModel

    @POST("/surveys/{id}")
    suspend fun getSurveysDetails(@Path("id") mApplianceId: String): TokenResponse

    @POST("/responses")
    suspend fun submitSurveys(): TokenResponse
}